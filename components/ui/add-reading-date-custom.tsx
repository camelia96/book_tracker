...
export function AddReadPagesDate({ bookData, sumReadPages, onReadingDateCreated }: AddReadingDateDialogProps) {
  ...
  const onSubmit = async (values: z.infer<typeof formSchema>) => {
    // Add reading date
    const result = await createReadingDate(bookData.books_profiles[0].id, values.date, values.pages);

    if (result && result.createdReadingDate) {
      // Pass data to parent
      onReadingDateCreated(result.createdReadingDate)

      toast.success("New reading date successfully created")
    } else {
      toast.error("There was a problem trying to add a new reading date")
    }
    ...
    <Dialog>
      ...
      <DialogContent className="sm:max-w-[425px]">
        {error &&
          (<AlertCustom
            type={"destructive"}
            title="Error - New reading date not created"
            description="There's been an error while adding a new reading date. Try again later."
            color="error" />)}
